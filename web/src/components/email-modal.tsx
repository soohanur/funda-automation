...
function EmailModalInner({ property, onClose }: { property: Property; onClose: () => void }) {
  ...
  return (
    <div className="fixed inset-0 z-50 grid place-items-center overflow-y-auto bg-black/50 p-3 sm:p-4">
      <div className="card flex max-h-[90dvh] w-full max-w-2xl flex-col overflow-hidden">
        <div className="flex shrink-0 items-center justify-between border-b border-[var(--border)] p-4 sm:p-5">
          ...
          <button
            type="button"
            onClick={onClose}
            className="grid h-8 w-8 shrink-0 place-items-center rounded-lg text-[var(--muted-foreground)] hover:bg-[var(--muted)]"
            aria-label="Close"
          >
            <X className="h-4 w-4" />
          </button>
        ...
        <form onSubmit={onSend} className="flex-1 space-y-4 overflow-y-auto p-4 sm:p-5">
          <div>
